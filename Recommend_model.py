...
class Recommend_model:
    def __init__(self, n, model_type, ensure_new=True):
        ...
        self.name = 'model_{}_n_{}'.format(model_type, n)
        self.ensure_new = ensure_new

    def fit(self, train_data):
        self.users = {user_id: {} for user_id in pd.unique(train_data['visitorid'])}
        self.items = {item_id: {} for item_id in pd.unique(train_data['itemid'])}
    ...
    def load(self):
        ...
        try:
            ...
            raise OSError(
                '{} has not trained and saved yet!'.format(self.name))
